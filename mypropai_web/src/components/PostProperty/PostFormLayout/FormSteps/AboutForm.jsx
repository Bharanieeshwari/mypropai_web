...
import { getOptionsWithCustomValue } from "@/utils/helpers";
// import api
import { incrementPostProperty, postProperty } from "@/lib/property";

export const InputFocus =
  "border-none outline-none shadow-none focus-visible:outline-none focus-visible:ring-1  focus-visible:ring-black  focus-visible:border-none";
...
const AboutForm = ({
  ...
  const getDefaultValues = (formData) => ({
    ...
    plotfloors: formData?.details?.plotfloors || "",
    parkingCount_covered: formData?.details?.parkingCount_covered,
    parkingCount_opened: formData?.details?.parkingCount_opened,
    projectStatus: formData.details?.projectStatus || "",
    PropertyType: formData.details?.PropertyType || "",
    buildArea: formData.details?.buildArea || "",
    buildAreaUnit: formData.details?.buildAreaUnit || "Sq.ft",
  });

  const schema = getSchema(subPropertyType);
  ...
  return (
    <div className="mt-7 mx-7 bg-[#ECEAFF] w-[520px] rounded-lg p-[30px]">
      <h2 className="text-[24px] font-bold mb-6">About Your Property</h2>
      {loading && (
        ...
        <form onSubmit={handleSubmit(onSubmit, onError)}>
              ...
              render={({ field, fieldState }) => (
                <>
                  <InputField
                    ...
                    inputClass={`mt-3 w-full h-10 rounded-md border-3 border-red-500 focus:ring-1 focus:ring-red-500 bg-white px-4 py-2 text-base text-gray-900 placeholder:text-gray-400`}
                    onChange={(e) => {
                  ...
                  {field.value?.length === 200 && !fieldState.error && (
                    <p className="text-red-500 text-sm mt-2 ml-1">
                      Tittle limit reached 200 characters
                    </p>
                  )}
                </>
              )}
          ...
          <div className="mt-6">
            <Label
              ...
              className="block text-lg font-medium  text-[#36334D]"
            >
              Property Description
            </Label>
            <Controller
              ...
              render={({ field }) => (
                <Textarea
                  ...
                  className={`mt-3 h-[90px] w-full rounded-md border-0 bg-white px-4 pt-4 text-base text- text-[#36334D] placeholder:text-gray-400 ${InputFocus}`}
                  onChange={(e) => {
          ...
          <div className="mt-4 flex items-center gap-2 text-sm font-medium text-[#36334D]">
            <AlertCircle className="h-5 w-5  text-[#36334D] border-0" />
          ...
          <div className="flex flex-row justify-between ">
            ...
            {errors?.landAreaUnit && (
              <span className="text-red-500 text-sm mr-2  mt-2 ml-1">
          ...
          {/* Dimensions */}
          {shouldShowField("dimension", subPropertyType) && (
            <>
                    ...
                    <>
                      <InputField
                        ...
                        onChange={(e) => {
                          ...
                          field.onChange(val);
                          updateUnChangedField("price", val);
                        }}
                      />
            ...
            <Controller
              ...
              render={({ field }) =>
                priceTypeOptions.map((typeObj, idx) => (
                  ...
                  >
                    <div
                      className={`h-5 w-5 border-2 rounded-sm flex items-center justify-center  ${field.value === typeObj.key
          ...
          {errors.priceOption && (
            <span className="text-red-500 text-sm  mt-2 ml-1">
          ...
          {shouldShowField("lookingfor", subPropertyType) && (
            <>
          ...
          {shouldShowField("facing", subPropertyType) && (
            <>
          ...
          {/* Approval */}
          {shouldShowField("approval", subPropertyType) && (
            <>
          ...
          {/* build up area */}
          {shouldShowField("buildupArea", subPropertyType) && (
            <>
              <LabelField
                htmlfor="buildupArea"
              ...
              <div className="relative">
                <Controller
                  name="buildArea"
                  control={control}
                  defaultValue={formData.details?.buildArea || "sq_ft"}
                  render={({ field }) => (
                    <InputField
                      {...field}
                      placeholder="Enter your buildup area "
                      type="text"
                      onChange={(e) => {
                        const val = e.target.value.replace(/[^0-9]/g, "");
                        field.onChange(val);
                        updateField("buildArea", val);
                      }}
                      onInput={(e) => {
                ...
                <div className="absolute top-0 right-0 pr-3">
                  <Controller
                    ...
                    control={control}
                    defaultValue={
                      formData.details?.buildAreaUnit?.unit || "sq_ft"
                    }
                    render={({ field }) => (
                      <SelectField
                        {...field}
              ...
              <div className="flex flex-row justify-between ">
                ...
                {errors?.buildAreaUnit && (
                  <span className="text-red-500 text-sm mr-2 mt-2">
          ...
          {/* bhk */}
          {shouldShowField("bhk", subPropertyType) && (
            <>
          ...
          {/* Rooms,Bedroom,Balcony,Bathroom*/}
          {shouldShowField("RoomDetails", subPropertyType) && (
            <>
              <div className="space-y-6 mt-4">
                <div className="space-y-6 mt-4">
                  {Object.entries(RoomDetails).map(([key, options]) => {
                    const formValue = formData?.details?.[key];
                    const finalOptions = getOptionsWithCustomValue(
                      options,
                      formValue,
                      isEdit
                    );
                    ...
                    return (
                      <div key={key} className="my-4">
                        <label
                          className="text-[16px] mt-2 inline-block"
                          htmlFor={key}
                        >
                          {key
                            .replace(/([A-Z])/g, " $1")
          ...
          {shouldShowField("floors", subPropertyType) && (
            <>
              <Label
                className="text-[16px] mt-6 inline-block"
                htmlFor="totalFloors"
              >
                Total no. of floors
              </Label>
          ...
          {/* parking */}
          {shouldShowField("parking", subPropertyType) && (
            <>
              <Label className="text-[16px] mt-6">Parking</Label>
              {parkingTypes.map(({ label, key }) => (
                <Controller
                  key={key}
                  name={`parkingCount_${key}`}
                  control={control}
                  rules={{ required: `${label} is required` }}
                  defaultValue={formData.details?.[`parkingCount_${key}`] || 0}
                  render={({ field }) => (
                    <ParkingCounter
                      label={label}
                      value={field.value || 0}
                      onChange={(val) => {
                        field.onChange(val);
                        updateField(`parkingCount_${key}`, val);
                      }}
                    />
                  )}
                />
          ...
          {/* Project Status */}
          {shouldShowField("projectstatus", subPropertyType) && (
            <>
              <LabelField
          ...
          {isEdit ? (
            ...
            <Button type="submit" className={`${PostPropertyButton} mt-0`}>
              Post the Property
            </Button>
          )}
        </form>
